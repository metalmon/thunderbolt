    ...
    <div className={cn('flex flex-col gap-3', !isMobile && 'w-[19rem]')}>
      <div className="relative">
        ...
        <Input
          ...
          aria-label={t('emoji.searchAriaLabel')}
      ...
      {catalog === 'failed' ? (
        ...
        <p className="py-6 text-center text-[length:var(--font-size-sm)] text-muted-foreground">{t('emoji.loading')}</p>
      ) : rows.length === 0 ? (
        <p className="py-6 text-center text-[length:var(--font-size-sm)] text-muted-foreground">{t('emoji.noMatches')}</p>
      ) : (
        <div className="relative">
          {pinnedLabel && (
...
export const EmojiPicker = ({ value, onChange, label }: EmojiPickerProps) => {
  ...
  const trigger = (
    <Button
      ...
      aria-label={t('emoji.chooseIconFor', { label })}
  ...
  if (isMobile) {
    return (
      <>
        {trigger}
        <MobileActionSheet open={open} onOpenChange={setOpen} title={t('emoji.chooseIcon')}>

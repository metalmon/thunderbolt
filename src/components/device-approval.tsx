    ...
    <ApprovalShell>
      {state.status === 'verifying' && (
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl">{t('deviceApproval.verifyingTitle')}</DialogTitle>
          <DialogDescription className="text-center">{t('deviceApproval.verifyingDescription')}</DialogDescription>
        </DialogHeader>
      )}
      ...
      {state.status === 'enteringCode' && (
        <form onSubmit={submitCode}>
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">{t('deviceApproval.enterTitle')}</DialogTitle>
            <DialogDescription className="text-center">{t('deviceApproval.enterDescription')}</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-2 py-4">
            <Label htmlFor="device-user-code">{t('deviceApproval.codeLabel')}</Label>
            <Input
              id="device-user-code"
              autoFocus
              autoComplete="off"
      ...
      {(state.status === 'confirming' || isSubmitting) && (
        <>
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">{t('deviceApproval.confirmTitle')}</DialogTitle>
            <DialogDescription className="text-center">{t('deviceApproval.confirmDescription')}</DialogDescription>
          </DialogHeader>
      ...
      {state.status === 'approved' && (
        <>
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">{t('deviceApproval.approvedTitle')}</DialogTitle>
            <DialogDescription className="text-center">{t('deviceApproval.approvedDescription')}</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Button className="w-full" onClick={goHome}>
              {t('deviceApproval.done')}
            </Button>
          </div>
        </>
      )}
      ...
      {state.status === 'denied' && (
        <>
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">{t('deviceApproval.deniedTitle')}</DialogTitle>
            <DialogDescription className="text-center">{t('deviceApproval.deniedDescription')}</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Button variant="outline" className="w-full" onClick={goHome}>
              {t('deviceApproval.close')}
            </Button>
          </div>
        </>
      )}
        ...
        <>
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">
              {state.error.reason === 'expired' ? t('deviceApproval.failedExpired') : t('deviceApproval.failedInvalid')}
            </DialogTitle>
            <DialogDescription className="text-center">{state.error.message}</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-2 py-4">
            <Button variant="outline" className="w-full" onClick={reset}>
              {t('deviceApproval.enterDifferent')}
            </Button>
    ...
    return (
      <ApprovalShell>
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl">{t('deviceApproval.loadingTitle')}</DialogTitle>
          <DialogDescription className="text-center">{t('deviceApproval.loadingDescription')}</DialogDescription>
        </DialogHeader>
      </ApprovalShell>
    )
  }

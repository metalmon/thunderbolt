...
**Scope:** Thunderbolt app UI + built-in defaults display names in the `metalmon/thunderbolt` fork, integrated with the Zeroclaw-style `main` + cherry-pick → `master` pipeline
...
- Patch layout compatible with `rebuild-master`: infra / wrap-* / locales as separate `$Branches` entries
